...
exports.validateSkillTraining = validateSkillTraining;
exports.getTrainableSkills = getTrainableSkills;
exports.getNextSkills = getNextSkills;
exports.calculateSkillProgress = calculateSkillProgress;
exports.getSkillMasteryLevel = getSkillMasteryLevel;
const skill_definitions_1 = require("./skill-definitions");
/**
 * Validate if a horse can train a skill
 */
function validateSkillTraining(horse, skillId) {
    ...
    if (skill.prerequisites && skill.prerequisites.length > 0) {
        ...
        for (const prereq of skill.prerequisites) {
            ...
            if (prereq.anyOf) {
                ...
                if (!hasAny) {
                    const options = prereq.anyOf.map(opt => `${skill_definitions_1.SKILLS[opt.skill]?.name || opt.skill} ${opt.minLevel}%`).join(' OR ');
                ...
                if (currentLevel < prereq.minLevel) {
                    missingPrereqs.push(`${skill_definitions_1.SKILLS[prereq.skill]?.name || prereq.skill} (need ${prereq.minLevel}%, have ${currentLevel.toFixed(1)}%)`);
                }
            }
        }
 ...
 */
function getTrainableSkills(horse) {
 ...
 */
function calculateSkillProgress(horse) {
    const { SKILLS } = require('./skill-definitions');
 ...
 */
function getSkillMasteryLevel(level) {
    if (level >= 90)
        return 'Master';
    if (level >= 75)
        return 'Expert';
