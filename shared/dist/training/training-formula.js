...
exports.applyTraining = applyTraining;
const training_types_1 = require("../types/training.types");
const mental_state_types_1 = require("../types/mental-state.types");
const skill_definitions_1 = require("../skills/skill-definitions");
const mood_system_1 = require("./mood-system");
 ...
 */
function applyTraining(horse, skillId, duration, trainer) {
    const skill = (0, skill_definitions_1.getSkill)(skillId);
    if (!skill) {
        return {
            success: false,
            skillGained: 0,
    ...
    if (skill.category === 'care') {
        ...
        fatigueGained = (0, mood_system_1.updateFatigue)(horse.mentalState.fatigue, duration, skill.isPhysical) - horse.mentalState.fatigue;
    }
    // Check for mood transitions
    let moodChanged = false;
    let newMood = horse.mentalState.mood;
    // Check for confusion
    if ((0, mood_system_1.checkForConfusion)(horse.mentalState.mood, currentSkillLevel)) {
        moodChanged = true;
        newMood = 'Confused';
    }
    else {
        // Check for other mood transitions
        const transition = (0, mood_system_1.checkMoodTransition)(horse, skillId, skill.isPhysical);
        if (transition) {
            moodChanged = true;
            newMood = transition;
        }
 ...
 */
function calculateSessionValue(horse, skillId, currentSkillLevel, trainer, duration) {
    const skill = (0, skill_definitions_1.getSkill)(skillId);
    if (!skill)
        return 0;
    // FM - Fatigue Modifier (0.0 - 1.0)
    const FM = (0, mood_system_1.getFatigueModifier)(horse.mentalState.fatigue);
 ...
 */
function calculatePrerequisiteModifier(horse, skill) {
 ...
 */
function getMoodModifier(mood, isPhysical, currentSkill, previousSkill) {
    ...
    return mental_state_types_1.MOOD_MODIFIERS[mood] || 1.0;
}
/**
 * Get highest bond level with a person
 */
function getHighestBondLevel(horse, trainerId) {
 ...
 */
function updateStatsFromSkill(horse, contributions, skillGained) {
    const statsGained = {};
